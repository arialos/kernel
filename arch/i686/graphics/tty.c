...
void ttyClear() {
    for ( size_t y = 0; y < vgaHeight; y++ ) {
        for ( size_t x = 0; x < vgaWidth; x++ ) {
            terminalBuffer[y * vgaWidth + x] =
                ttyEntry( ' ', ttyColor( ttyFgColor, ttyBgColor ) );
        }
    }
}
...
void ttyPutCharactor( char c ) {
    ...
    terminalBuffer[currentRow * vgaWidth + currentColumn] =
        ttyEntry( c, ttyColor( ttyFgColor, ttyBgColor ) );
}

void ttyPutString( const char *data ) {
    for ( size_t i = 0; i < strlen( data ); i++ ) ttyPutCharactor( data[i] );
}
